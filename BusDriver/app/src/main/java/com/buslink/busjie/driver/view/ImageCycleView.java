...
public class ImageCycleView extends LinearLayout {
    ...
    public void setImageResources(List<String> imageUrlList,ImageCycleViewListener imageCycleViewListener) {
        ...
        for (int i = 0; i < imageCount; i++) {
            ...
            mImageViews[i].setImageResource(R.drawable.s_circle);
            if (i == 0) {
                mImageViews[i].setColorFilter(Color.GREEN);//设置小圆点的颜色为绿色
            } else {
                mImageViews[i].setColorFilter(Color.GRAY);
            }
    ...
    private final class GuidePageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageSelected(int index) {
            ...
            mImageIndex = index;
            // 设置图片滚动指示器背
            mImageViews[index].setColorFilter(Color.GREEN);//小圆点滚动后到达的位置的点
